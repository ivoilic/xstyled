...
function getVariants(props: IProps) {
  ...
  return { ...medias, ...getStates(props) }
}

function getCachedVariants(props: IProps, cache: ThemeCache) {
...
export function reduceVariants(
  ...
  const variants = cache ? getCachedVariants(props, cache) : getVariants(props)
  let styles: IStyles = {}
  for (const value in values) {
    ...
    if (variant === null) {
      ...
      styles[variant] = styles[variant] ? assign(styles[variant], style) : style
    }
  }
  return styles
}
...
function getStyleFactory(
  ...
  return function getStyle(props: IProps) {
    const fromValue = (value: any) => {
      ...
      if (obj(value)) {
        return reduceVariants(props, value, fromValue, cache)
      }
...
export function compose(...generators: StyleGenerator[]): StyleGenerator {
  ...
  function getStyle(props: IProps, sort = true) {
    ...
    const medias = getCachedVariants(
      props,
      getCacheNamespace(props.theme, '__variants'),
    )
